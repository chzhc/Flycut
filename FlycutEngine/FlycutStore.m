...
//  homepage at <https://github.com/TermiT/Flycut> for details.
//
//
...
- (id)init {
    return [self initRemembering:20 displaying:10 withDisplayLength:40];
}

- (id)initRemembering:(int)nowRemembering
           displaying:(int)nowDisplaying
    withDisplayLength:(int)displayLength {
    // Ensure that we don't remember zero or fewer clippings.  Very unlikely,
    // but this provides a safeguard just in case.
    if (nowRemembering <= 0)
        nowRemembering = 40;
...
- (int)indexOfClipping:(NSString *)clipping
                  ofType:(NSString *)type
    fromAppLocalizedName:(NSString *)appLocalizedName
        fromAppBundleURL:(NSString *)bundleURL
             atTimestamp:(int)timestamp {
...
- (int)indexOfClipping:(FlycutClipping *)clipping afterIndex:(int)after {
    NSUInteger index = [jcList
        indexOfObject:clipping
...
// Add a clipping
- (bool)addClipping:(NSString *)clipping
                  ofType:(NSString *)type
    fromAppLocalizedName:(NSString *)appLocalizedName
        fromAppBundleURL:(NSString *)bundleURL
             atTimestamp:(int)timestamp {
...
- (bool)removeDuplicates {
    return [[[NSUserDefaults standardUserDefaults]
        valueForKey:@"removeDuplicates"] boolValue];
}
...
- (void)insertClipping:(FlycutClipping *)clipping atIndex:(int)index {
    ...
    if (index < [jcList count]) {
        ...
        // If the index is beyond the current count then just append it and
        // disregard requested index. This doesn't alter the remember number and
        // the jcList is self-growing so it is fine to append.
        index = [jcList count];
    ...
    while ([jcList count] > jcRememberNum) {
        ...
        [self delegateDeleteClippingAtIndex:(jcRememberNum -
                                             1)]; // -1 for before-add indexing
    }

    [self delegateEndUpdates];
}

- (void)addClipping:(NSString *)clipping
             ofType:(NSString *)type
        withPBCount:(int *)pbCount {
    [self addClipping:clipping
                      ofType:type
        fromAppLocalizedName:@"PBCount"
...
- (void)mergeList {
    NSString *merge = [[[[jcList reverseObjectEnumerator] allObjects]
        valueForKey:@"clipContents"] componentsJoinedByString:@"\n"];
    [self addClipping:merge
                      ofType:NSStringFromClass([merge class])
        fromAppLocalizedName:@"Merge"
            fromAppBundleURL:nil
                 atTimestamp:0];
}
...
- (NSString *)clippingContentsAtPosition:(int)index {
    if (index >= [jcList count]) {
        ...
        return
            [NSString stringWithString:[[jcList objectAtIndex:index] contents]];
    }
}
...
- (NSString *)clippingTypeAtPosition:(int)index {
    ...
    returnString =
        [NSString stringWithString:[[jcList objectAtIndex:index] type]];
...
- (NSArray *)previousDisplayStrings:(int)howMany containing:(NSString *)search {
    ...
    // 确保jcList存在，非空检查
    if (!jcList || [jcList count] == 0) {
        return returnArray;
    }

    // 如果提供了搜索字符串，执行搜索逻辑
    if (nil != search && search.length > 0) {
        subArray = [jcList copy];  // 使用拷贝的jcList以避免数据变动影响
        NSEnumerator *enumerator = [subArray objectEnumerator];
        FlycutClipping *aClipping;
        int index = 0; // 配合enumerator同步更新索引，循环遍历jcList
         
        while ((aClipping = [enumerator nextObject])) {
            // 使用刚设置的index进行检查，不依赖other的子函数
        ...
        [subArray release]; // 释放拷贝的数组
        // 返回从前到后的原始数组顺序
        return [[returnArray reverseObjectEnumerator] allObjects];
    }
     
    // 没有搜索条件时
    int jcListCount = (int)[jcList count];
    theRange.location = MAX(0,
                            jcListCount - howMany);
    theRange.length = MIN(howMany,
                          jcListCount);
    // 获取需要的子区间
    subArray = [jcList subarrayWithRange:theRange];
...
- (NSArray *)previousIndexes:(int)howMany
                  containing:(NSString *)
                                 search // This method is in newest-first order.
{
    NSArray *subArray;
    NSMutableArray *returnArray = [[[NSMutableArray alloc] init] autorelease];
    NSEnumerator *enumerator;
    ...
    if (nil != search && search.length > 0) {
        ...
        [subArray release]; // Release copied subArray
    } else {
...
- (void)delegateBeginUpdates {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(beginUpdates)])
        [self.delegate beginUpdates];
}

- (void)delegateEndUpdates {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(endUpdates)])
...
- (void)delegateInsertClippingAtIndex:(int)index {
    ...
    if ([insertionJournal count] > jcRememberNum)
        [self clearInsertionJournalCount:([insertionJournal count] -
                                          jcRememberNum)];

    modifiedSinceLastSaveStore = YES;
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(insertClippingAtIndex:)])
...
- (void)delegateWillDeleteClippingAtIndex:(int)index {
    ...
    if ([deletionJournal count] > jcRememberNum)
        [self clearDeletionJournalCount:([deletionJournal count] -
                                         jcRememberNum)];

    if (self.deleteDelegate &&
        [self.deleteDelegate
            respondsToSelector:@selector(willDeleteClippingFromStore:AtIndex:)])
...
- (void)delegateDeleteClippingAtIndex:(int)index {
    modifiedSinceLastSaveStore = YES;
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(deleteClippingAtIndex:)])
...
- (void)delegateReloadClippingAtIndex:(int)index {
    modifiedSinceLastSaveStore = YES;
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(reloadClippingAtIndex:)])
